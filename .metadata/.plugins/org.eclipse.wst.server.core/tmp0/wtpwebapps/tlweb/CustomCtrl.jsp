		...
		<div id="question">
			...
			<div style="clear:both;margin-top:20px;">
				<table cellspacing="5">
					...
					<tr><td><h4>使用要求</td></tr>
					<tr><td>1.使用前请仔细阅读本产品使用说明书 2.使用过程中严禁违规操作 3.请确保操作人员已明确知晓重要安全说明</td></tr>
					<tr><td><h4>保修事项</td></tr>
					<tr><td>1.对于违规操作产生的故障不予以保修 2.保修起始日期为注册日期 3.如注册日期与出厂日期相差15日按出厂日期计算保修日期 4.未在官网激活设备将不予以保修</td></tr>
					<tr><td><h4>其他事项</td></tr>
...
function getRegisterResult() {
    if (http_request.readyState == 4) {     		// 判断请求状态
        if (http_request.status == 200) {  // 请求成功，开始处理返回结果
        	if(http_request.responseText==0){
        		document.getElementById("sntip").innerHTML="已注册成功";
        		document.getElementById("registerB").value="已注册";
        		createRequest('AjaxJs/ReturnService.jsp?service='+encodeURI("产品注册")+'&id='+<%=id%>+'&datetime='+new Date().getTime());
        	}else{
...
function clickButton(){
	...
	if(tip==""&&val!=""){
		if(checkEq){
			...
			document.getElementById("registerB").disabled=true; 
			document.getElementById("registerB").style.backgroundColor="gray";
			createRegisterRequest("AjaxJs/RegisterProduct.jsp?uid="+encodeURI(uid)+"&sn="+encodeURI(val)+"&datetime="+new Date().getTime());
		}else{
			//alert("未收到Ajax返回值");
		}
	}else{
