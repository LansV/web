	...
	<div id="showques">
		<div id="question">
			<div id="questop" style="width:auto;height:60px;">
				...
				placeholder="请输入设备SN码  例:MF1604060A" type="text" maxlength=20 onblur="checkSN(this)">
			...
			<div style="clear:both;margin-top:20px;">
				<table cellspacing="5">
					...
					<tr><td><h4>其他事项</td></tr>
					<tr><td>1.收货前请与经销商确认出厂日期 2.保修详细：电机及泵保修365日；电器、电气设备保修30日</td></tr>
					<tr></tr>
					<tr></tr>
					<tr></tr>
					<tr></tr>
					<tr>
					<tr><td><input  type="checkbox" id="agreement" onclick="checkUserAgree(this)">已仔细阅读上述要求,并同意遵守</td></tr>
					<tr>
				</table>
			</div>
			<input id="registerB" style="position:absolute; border:0px;color:white;font-size:20px;border-radius:3px;
			width:300px;height:45px;background-color:gray;left:300px;bottom:30px;" type="button"  value="注册" onmousemove="getFocus()" disabled=true>
		</div>
	</div>
...
var start=createRequest('AjaxJs/ReturnService.jsp?service='+encodeURI("产品注册")+'&datetime='+new Date().getTime());
...
function getCheckSNResult() {
    if (http_request.readyState == 4) {     		// 判断请求状态
        if (http_request.status == 200) {     		// 请求成功，开始处理返回结果
            document.getElementById("sntip").innerHTML=http_request.responseText;	//设置提示内容
            //alert(http_request.responseText);
           // document.getElementById("toolTip").style.display="block";	//显示提示框
        } else {     						// 请求页面有错误
            alert("您所请求的页面有错误！");
